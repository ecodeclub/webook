...
func NewCreditConsumer(svc service.Service, q mq.MQ) (*CreditConsumer, error) {
	groupID := "credit"
	consumer, err := q.Consumer(creditEvents, groupID)
	if err != nil {
		return nil, err
	}
...
func (c *CreditConsumer) Consume(ctx context.Context) error {
