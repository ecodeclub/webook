...
func InitModule(db *gorm.DB, q mq.MQ, c ecache.Cache, om *order.Module, pm *product.Module) (*Module, error) {
	...
	wechatRobotEventProducer, err := producer.NewQYWeChatEventProducer(q)
	if err != nil {
		return nil, err
	}
	service3 := service.NewService(marketingRepository, service2, serviceService, v, v2, memberEventProducer, creditEventProducer, permissionEventProducer, wechatRobotEventProducer)
